...
# Define landmark indices for different points on the face
bottom_nose_idx = 2
top_lip_idx = 13
bottom_lip_idx = 14
chin_idx = 152
...
# Add this function right after the landmark indices section, around line 45
def setup_ratio_points(face_landmarks):
    global ratio_points
    ratio_points = {
        # Existing points...

        # Calculate the highest point with a 10% offset above the forehead
        'top_forehead': calculate_highest_point_with_percentage_offset(face_landmarks, height, width),

        # Calculate midpoint between inner eyebrows
        'eyebrow_midpoint': (
...
# Function to calculate the highest point with a 10% offset above the forehead landmark (landmark 10)
...
# Function to load and process the image for display
def load_image(image_path):
    global loaded_image, width, height
    loaded_image = cv2.imread(image_path)
    height, width, _ = loaded_image.shape

# Function to calculate and display Nose-Lips-Chin Ratio
...
# Function to calculate and display Eye Symmetry Ratio
...
# Function to calculate and display both ratios
...
# Function to calculate and display Face Height-to-Width Ratio
...
# Function to calculate and display Nose - Chin / Forehead Ratio
def show_cd_ratio():
    global loaded_image
    if loaded_image is None:
        return
...
# Function to calculate and display Eye Pupil to Width Ratios
...
# Function to calculate and display Mouth / Nose Ratio
def show_mouth_nose_ratio():
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            # Define landmarks for mouth corners and nose wings
            ...
            # Get coordinates for nose wings
            ...
            # Draw lines for visualizing the widths on the image
...
def show_face_sections(image):
    ...
    # Check if ratio_points is initialized, if not, call setup_ratio_points
    rgb_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    results = face_mesh.process(rgb_image)
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            # Calculate the lengths of each face section
            ...
            # Draw the lines for each section
            ...
            # Display the lengths of each section
    ...
    # Function to calculate and display Jawline-to-Face Width Ratio
def show_jawline_to_face_width_ratio():
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                # Define the jawline landmarks (left to right along the jawline)
                ...
                # Calculate jawline width by summing distances between adjacent jaw points
                jawline_width = sum(
                    math.sqrt(
                ...
                # Calculate face width (distance between landmarks 234 and 454)
...
# Ideal values for the golden ratio comparison
IDEAL_VALUES = {
...
def analyze_ratios():
    ...
    # Call each calculation function and store the results in a dictionary
    ratios = {
    ...
    # Calculate deviation for each ratio and compile results
    for ratio_name, actual_value in ratios.items():
        ...
        # Format each ratio and deviation
    ...
    # Calculate the Golden Ratio Score as the average deviation
    average_deviation = sum(deviations) / len(deviations)
    golden_ratio_score = 100 - average_deviation

    # Add the final Golden Ratio Score prominently
    result_output += "=" * 35 + "\n"
    result_output += f"Altın Oran Skoru: {golden_ratio_score:.2f}%\n"
    result_output += "=" * 35

    # Update the Tkinter label text
...
# Tkinter variable to hold the result text
result_text = tk.StringVar()

# Button to analyze and display ratios
